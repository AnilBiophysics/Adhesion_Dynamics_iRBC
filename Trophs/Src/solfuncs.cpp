...
//sorting solvent initially inside and outside after tricm
void sortsolvent_initial1()
{
   ...
   {	
	...
	//finding sold
	for ( int i = 1 ; i < solvent.size() ; ++i )
	{
		if ( (solvent[i-1].indio == 0 ) && (solvent[i].indio == 1) )
		{
			sold = i ;
			break ;
		}
	}
	cout << "index that divides in and out solvent=\t" << sold << endl ;
	
	
	//neighbour list computation
	for ( int i = 0 ; i < edges.size(); ++i )
	{
		int l1 = edges[i].l1 ;
		int l2 = edges[i].l2 ;
		vts[l1].nnlist.push_back(l2) ;
		vts[l2].nnlist.push_back(l1) ;
	}
	vector <int> temp ;
	for ( int i = 0 ; i < vts.size() ; ++i )
	{
		for ( int j = 0 ; j < vts[i].nnlist.size() ; ++j )
		{
			temp.push_back(vts[i].nnlist[j]) ;
		}
		for ( int j = 0 ; j < temp.size() ; ++j )
		{
			int k = temp[j] ;
			vts[i].nlist.insert(vts[i].nlist.end(),vts[k].nnlist.begin(),vts[k].nnlist.end()) ;
		}
		temp.clear() ;
	}
	...
	for ( int i = 0 ; i < vts.size() ; ++i )
	{
		sort( vts[i].nlist.begin(), vts[i].nlist.end() );
		vts[i].nlist.erase( unique( vts[i].nlist.begin(), vts[i].nlist.end() ), vts[i].nlist.end() );
	}	
}
...
{ 
	...
	rpos[0] = -a*0.5 + a*RanGen[nrt].Random() ;
	rpos[1] = -a*0.5 + a*RanGen[nrt].Random() ;
	rpos[2] = -a*0.5 + a*RanGen[nrt].Random() ;
	...
	{
		fx = (int)floor(solvent[i].xpos - rpos[0] + 1.0) ; // because of PBC
		fy = (int)floor(solvent[i].ypos - rpos[1] + 1.0) ; // because of PBC
		fz = (int)floor(solvent[i].zpos - rpos[2] + 1.0) ; // because of PBC
		solvent[i].pid = fx*(int)(Lz+2)*(int)(Ly+2) + fy*(int)(Lz+2) + fz ; 
	...
	for ( int  i = 0 ; i < vts.size() ; ++i )
	{
		fx = (int)floor(vts[i].x - rpos[0] + 1.0) ; // because of PBC
		fy = (int)floor(vts[i].y - rpos[1] + 1.0) ; // because of PBC
		fz = (int)floor(vts[i].z - rpos[2] + 1.0) ; // because of PBC
		vts[i].pid = fx*(int)(Lz+2)*(int)(Ly+2) + fy*(int)(Lz+2) + fz  ; 
		int ii = vts[i].pid ;
		Cells[ii].mem.push_back(i) ;
	}
	/***********************************************/
			
	// Now first compute partial center of mass velocites of the cells cut of boundaries (only partially cut cells)
	/*******************************************************/
   ...
   { 			   
	   ...
	   {
			pi = Cells[ii].sol[i] ;
			Cells[ii].vcm[0] += solvent[pi].mas*solvent[pi].vx ;
			Cells[ii].vcm[1] += solvent[pi].mas*solvent[pi].vy ;
			Cells[ii].vcm[2] += solvent[pi].mas*solvent[pi].vz ;
			Cells[ii].ncc += (1*solvent[pi].mas) ;
			Cells[ii].nc += 1 ;
	   }
   }
	...
	#pragma omp parallel private(nn)
	{
	   ...
	   for ( int ii = 0 ; ii < Cells.size() ; ++ii )
	   {
		   for ( int i = 0 ; i <  Cells[ii].sol.size() ; ++i )
		   {
			   ...
			   Cells[ii].dvv += (solvent[pi].mas*aa) ;
		   }
	   }
	   //for mem beads
	   	#pragma omp parallel for private(pi,aa,A)
	...
	for ( int i = 0 ; i < Cells.size() ; ++i )
	{
		 ...
		 if ( Cells[i].dvv != 0 )
		 {
			 double simE = 0.5*Cells[i].dvv ;
			 double f2 = 3.0*((double)Cells[i].nc - 1.0)*0.5 ;
			 double expE = gsl_ran_gamma(f2,Theta) ;
			 Cells[i].ThE = sqrt(expE/simE) ;
		 }
	 }						
...
void llistupdate()
{
  ...
  double distsq = (0.5 + 1.0*avg_edge)*(0.5 + 1.0*avg_edge) ; 
  #pragma omp parallel
  {
	 #pragma omp for	
	for ( unsigned int i = 0 ; i < Cells.size() ; ++i )
	{ 
		...
		for ( unsigned int j = 0 ; j <  tries.size() ; ++j )
		{
			...
			double tx = fmod((((vts[p1].x + vts[p1].ix*Lx) +  (vts[p2].x  + vts[p2].ix*Lx)  + (vts[p3].x + vts[p3].ix*Lx))/3.0),Lx) ;
			double ty = fmod((((vts[p1].y + vts[p1].iy*Ly) +  (vts[p2].y  + vts[p2].iy*Ly)  + (vts[p3].y + vts[p3].iy*Ly))/3.0),Ly) ;
			double tz = ((vts[p1].z  +  vts[p2].z  +  vts[p3].z )/3.0) ;
			
			
			if ( ( cellx < 1.0 ) && (tx > Lx-1.0) )
			{
				cellx += Lx ;
			}
			if ( ( cellx > Lx-1.0 ) && (tx < 1.0) )
			{
				cellx -= Lx ;
			}
			if ( ( celly < 1.0 ) && (ty > Ly-1.0) )
			{
				celly += Ly ;
			}
			if ( ( celly > Ly-1.0 ) && (ty < 1.0) )
			{
				celly -= Ly ;
			}								
			
			double xxx1 =  tx - cellx ;
			double yyy1 =  ty - celly ;
			double zzz =   tz - cellz ;
...
double reflectionFed(int i,double rstep)
{
	...
	int fx = (int)floor(solvent[i].xpos + 1.0) ;
	int fy = (int)floor(solvent[i].ypos + 1.0) ; 
	int fz = (int)floor(solvent[i].zpos + 1.0) ;
	int pid = fx*(int)(Lz+2)*(int)(Ly+2) + fy*(int)(Lz+2) + fz ;
	for ( unsigned int j = 0 ; j < Cells[pid].llist.size() ; ++j )
	{
		flg = 0 ;
		int jj = Cells[pid].llist[j] ;
		double chx = fabs( solvent[i].xpos - tries[jj].cmx ) ;
		double chy = fabs( solvent[i].ypos - tries[jj].cmy ) ;
		double chz = fabs( solvent[i].zpos - tries[jj].cmz ) ;
		chx -= Lx*round(chx/Lx) ;
		chy -= Ly*round(chy/Ly) ;
		if ( ( chx < avg_edge ) && (chy < avg_edge) && (chz < avg_edge) )
		{
			int p1 = tries[jj].l1 ;
			int p2 = tries[jj].l2 ;
			int p3 = tries[jj].l3 ;	
			...
			/**************************************************/
			double rstencilx = Lx - (2.0*a) ;
			double lstencilx = 2.0*a ;
			double rstencily = Ly - (2.0*a) ;
			double lstencily = 2.0*a ;
