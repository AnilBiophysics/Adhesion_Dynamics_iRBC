...
void force_calculate_dt()
{
  ...
  #pragma omp parallel private(ffx,ffy,ffz) default(shared)
  {
	...
	for ( int i = 0 ; i < edges.size() ; ++i )
	{
		...
		A[1] /= aa ;
		A[2] /= aa ;
		B[0] = vts[l1].vx - vts[l2].vx ;
		B[1] = vts[l1].vy - vts[l2].vy ;
		B[2] = vts[l1].vz - vts[l2].vz ;
		double ab = A[0]*B[0] + A[1]*B[1] + A[2]*B[2] ;
		ffx[l1] += -memdis*ab*A[0] ;
		ffy[l1] += -memdis*ab*A[1] ;
		ffz[l1] += -memdis*ab*A[2] ;
		ffx[l2] += memdis*ab*A[0] ;
		ffy[l2] += memdis*ab*A[1] ;
		ffz[l2] += memdis*ab*A[2] ;	 

		
		double xx = aa/(mfactor*l0) ;
		double xxx = 1.0 / (4.0*(1.0 - xx)*(1.0 - xx)) ;
		ffx[l1] += (-Theta/(edges[i].elap))*( xxx - 0.25 + xx )*A[0] ;
		ffy[l1] += (-Theta/(edges[i].elap))*( xxx - 0.25 + xx )*A[1] ;
		ffz[l1] += (-Theta/(edges[i].elap))*( xxx - 0.25 + xx )*A[2] ;
		ffx[l2] += (Theta/(edges[i].elap))*( xxx - 0.25 + xx )*A[0] ;
		ffy[l2] += (Theta/(edges[i].elap))*( xxx - 0.25 + xx )*A[1] ;
		ffz[l2] += (Theta/(edges[i].elap))*( xxx - 0.25 + xx )*A[2] ;
	...
	for ( int i = 0 ; i <  tries.size() ; ++i )
	{
		...
		for (int i = 0 ; i <  edges.size() ; ++i )
		{
			...
			if ( fabs(sinth) < 0.025)
			{
				betab = kappab*cos(theta0*Pi/180.0) ;
			}
			else
			{
				betab = kappab*(sinth*cos(theta0*Pi/180.0) - costh*sin(theta0*Pi/180.0))/sinth ;
			}
...
void Choose_Knob_Locations(int knob_size, double Center[])
{
	int ll, ll1, ll2 ;
	vector < double > parea ;
	...
	int knob1 = (int)(knob_size*0.1) ;
	int knob2 = knob_size - knob1 ;

	int knob_count = 0 ;


	while ( knob_count < knob1 )
	{
		...
		for ( int j = 0 ; j < parea.size() ; ++j )
		{
			...
			if ( aaa > rad )
			{
				...
				if ( radi == 1) {ll = tries[j].l1 ; ll1 = tries[j].l2 ; ll2 = tries[j].l3 ;}
				if ( radi == 2) {ll = tries[j].l2 ; ll1 = tries[j].l3 ; ll2 = tries[j].l1 ;}
				if ( radi == 3) {ll = tries[j].l3 ; ll1 = tries[j].l1 ; ll2 = tries[j].l2 ;}
	...
	while ( knob_count < knob2 )
	{
		...
		for ( int j = 0 ; j < parea.size() ; ++j )
		{
			...
			if ( aaa > rad )
			{
				...
				if ( (vts[ll].knob == 0) && (vts[ll1].knob == 1 || vts[ll2].knob == 1) )
				{
					vts[ll].knob = 1 ;
					knob_count += 1 ;
					break ;
